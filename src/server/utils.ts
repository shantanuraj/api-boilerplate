 ...
 */
const prefixRoute = (prefix: string, route: ServerRoute): ServerRoute => {
  const trimmedPath = route.path.length > 1 && route.path.endsWith('/') ? route.path.slice(0, -1) : route.path;
 ...
 */
export const prefixRoutes = (prefix: string, routes: ServerRoute[]): ServerRoute[] => {
